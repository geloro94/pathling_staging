...
CMD ["sh", "-c", "dockerd-entrypoint.sh & while ! docker info > /dev/null 2>&1; do sleep 1; echo 'Waiting for Docker daemon to start...'; done && gunicorn -w 4 -b 0.0.0.0:8000 --certfile cert.pem --keyfile key.pem main:app"]
